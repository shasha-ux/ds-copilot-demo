...
export function extractConfluencePageId(rawUrl) {
  try {
    const url = new URL(String(rawUrl || '').trim());
    const pageIdParam = url.searchParams.get('pageId');
    if (pageIdParam && /^\d+$/.test(pageIdParam)) return pageIdParam;
    // Typical Confluence URL: /wiki/spaces/SPACE/pages/123456/Title
...
export async function fetchContextFromUrlWithHeaders(rawUrl, extraHeaders = {}) {
  const parsed = validateUrl(String(rawUrl || '').trim(), 'contextUrl');
  const raw = await fetchText(parsed.toString(), readTimeoutMs(), extraHeaders);
  const extracted = summarizeText(normalizeContextBody(raw));
  return {
    contextUrl: parsed.toString(),
    rawLength: raw.length,
...
export async function fetchConfluenceContextById({ pageId, cloudId, accessToken }) {
  const id = String(pageId || '').trim();
  if (!id) throw new Error('pageId is required');
  if (!cloudId) throw new Error('cloudId is required');
  if (!accessToken) throw new Error('accessToken is required');

  const url = new URL(`https://api.atlassian.com/ex/confluence/${cloudId}/wiki/rest/api/content/${id}`);
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Confluence content fetch failed: ${response.status}`);
    }
    const title = data?.title ? `제목: ${data.title}` : '';
    const storage = data?.body?.storage?.value || '';
    const plain = stripHtml(storage);
    const extracted = summarizeText([title, plain].filter(Boolean).join('\n'));
    return {
      pageId: id,
      title: data?.title || '',
      extractedText: extracted,
...
export async function searchConfluenceDocumentsWithHeaders({ query, baseUrl, headers }) {
  const q = String(query || '').trim();
  if (!q) throw new Error('query is required');
  const base = baseUrl ? new URL(String(baseUrl).trim()).origin : resolveConfluenceBaseUrl();
  if (!base) {
    throw new Error('DS_CONFLUENCE_BASE_URL is not configured');
  }

  const url = new URL('/wiki/rest/api/search', base);
  url.searchParams.set('cql', `type=page AND text ~ "${q.replace(/"/g, '\\"')}"`);
  url.searchParams.set('limit', '10');
  url.searchParams.set('expand', 'content.space,content.version');
...
export async function searchConfluenceDocumentsWithSession({ query, cloudId, accessToken, resourceUrl }) {
  const q = String(query || '').trim();
  if (!q) throw new Error('query is required');
  if (!cloudId) throw new Error('cloudId is required');
  if (!accessToken) throw new Error('accessToken is required');
  ...
  try {
    ...
    const results = Array.isArray(data.results) ? data.results : [];
    const resourceBase = resourceUrl ? new URL(resourceUrl).origin : '';
    return results.map((row) => {
      const content = row.content || {};
      const links = content._links || {};
      const webui = links.webui && resourceBase ? `${resourceBase}${links.webui}` : '';
      return {
        id: content.id || '',
        title: content.title || '(제목 없음)',
        url: webui,
